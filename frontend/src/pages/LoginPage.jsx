...
import { ArrowRight, Mail, Lock, Eye, EyeOff, X, AlertCircle, Loader } from "lucide-react";
import { AnimatePresence, motion, useInView } from "framer-motion";
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';

// --- CONFETTI Context (Required for parity imports/placeholders if referenced) ---
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", onSubmit, onSuccess, imageSrc = loginBg }) => {
      ...
      {modalStatus !== 'closed' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white/95 border border-stone-200 rounded-2xl p-8 w-[340px] flex flex-col items-center gap-4 mx-4 text-center shadow-2xl backdrop-blur-xl">
            {modalStatus === 'error' && <button onClick={closeModal} className="absolute top-4 right-4 p-1 text-stone-400 hover:text-stone-900 transition-colors"><X className="w-5 h-5" /></button>}
            {modalStatus === 'error' && <>
      ...
      <div className="w-full max-w-5xl min-h-[640px] bg-white/35 border border-stone-200/40 rounded-3xl backdrop-blur-2xl shadow-xl flex flex-col relative overflow-hidden z-10">
        ...
        <header className="flex items-center justify-between px-6 py-4 border-b border-stone-200/20 w-full text-stone-700 text-xs md:text-sm tracking-wide font-medium z-20">
          ...
          <div className="flex items-center gap-6 text-[11px] uppercase tracking-wider font-semibold text-stone-700">
            ...
            <a href="/register" className="px-4 py-1.5 rounded-full border border-stone-200 hover:bg-stone-100/50 hover:border-stone-300 transition-all text-stone-950">Sign Up</a>
          </div>
        </header>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center justify-center gap-6 w-full max-w-sm min-h-[460px] mx-auto p-6 sm:p-8 bg-white/45 border border-stone-200/60 rounded-2xl backdrop-blur-md shadow-xl transition-all">
              <form onSubmit={handleFormSubmit} className="w-full flex flex-col items-center gap-4">
                ...
                <BlurFade delay={0.7} className="mt-2 text-center text-xs text-stone-500">
                  ...
                  <a href="/register" className="text-orange-600 font-medium hover:underline">Create one</a>
                </BlurFade>
              </form>
            </fieldset>
          </div>
